...
class CircuitGenParameters:
    ...
    def append_unitary_error(
        ...
        circuit.append_operation(name, targets, prob)
    
    def append_depolarize_1(
            self,
            circuit: stim.Circuit,
            targets: List[int]
    ) -> None:
...
def finish_surface_code_circuit_with_discrete_faults(
        coord_to_index: Callable[[complex], int],
        data_coords: Set[complex],
        x_measure_coords: Set[complex],
        z_measure_coords: Set[complex],
    ...
    # Build the body of surface code cycles with faults where indicated
    # cycles: List[stim.Circuit] = []

    # Build the repeated actions that make up the surface code cycle
    def build_cycle_with_faults(r: int) -> stim.Circuit:
        if params.gate_faults is not None:
            faults = params.gate_faults[r]
            cycle_actions = stim.Circuit()
            params.append_begin_round_tick(cycle_actions, data_qubits)
            params.append_unitary_1(cycle_actions, "H", x_measurement_qubits)

            # Apply state preparation faults
            for q, f in faults["PREP"].items():
                q = int(q)
                if f != "I":
                    params.append_unitary_error(cycle_actions, f"{f}_ERROR", [q], 1)

            for k, targets in enumerate(cnot_targets):
                cycle_actions.append_operation("TICK", [])
                params.append_unitary_2(cycle_actions, "CNOT", targets)
                # Apply CNOT faults
                if faults["CNOT"][k] is not None:
                    for q, f in faults["CNOT"][k].items():
                        q = int(q)
                        if q in x_measurement_qubits:
                            if f[0] != "I":
                                params.append_unitary_error(cycle_actions, f"{f[0]}_ERROR", [q], 1)
                            if f[1] != "I":
                                params.append_unitary_error(cycle_actions, f"{f[1]}_ERROR", [p2q[q2p[q] + x_order[k]]], 1)
                        else:
                            if f[0] != "I":
                                params.append_unitary_error(cycle_actions, f"{f[0]}_ERROR", [p2q[q2p[q] + z_order[k]]], 1)
                            if f[1] != "I":
                                params.append_unitary_error(cycle_actions, f"{f[1]}_ERROR", [q], 1)

            cycle_actions.append_operation("TICK", [])
            params.append_unitary_1(cycle_actions, "H", x_measurement_qubits)
            cycle_actions.append_operation("TICK", [])

            # Apply measurement faults
            for q, f in faults["MEASURE"].items():
                q = int(q)
                if f != "I":
                    params.append_unitary_error(cycle_actions, f"{f}_ERROR", [q], 1)
            params.append_measure_reset(cycle_actions, measurement_qubits)
    ...
    params.append_reset(head, data_qubits, "ZX"[is_memory_x])
    params.append_reset(head, measurement_qubits)
    head += build_cycle_with_faults(0)
    for measure in sorted(chosen_basis_measure_coords, key=lambda c: (c.real, c.imag)):
        head.append_operation(
            "DETECTOR",
            [stim.target_rec(-len(measurement_qubits) + measure_coord_to_order[measure])],
    ...
    for r in range(1, params.rounds):
        body += build_cycle_with_faults(r)
        m = len(measurement_qubits)
        body.append_operation("SHIFT_COORDS", [], [0.0, 0.0, 1.0])
        for m_index in measurement_qubits:
            m_coord = q2p[m_index]
...
def generate_rotated_surface_code_circuit(
    ...
    if params.gate_faults is not None:
        params.after_clifford_depolarization = 0  # gate faults are deterministic, given by params.gate_faults
        return finish_surface_code_circuit_with_discrete_faults(
            coord_to_idx,
            data_coords,
            x_measure_coords,
            z_measure_coords,
